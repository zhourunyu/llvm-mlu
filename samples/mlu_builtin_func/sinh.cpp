...
int main(){
    ...
    for(int i = 0;i < N;i++){
        a[i] = 1;b[i] = 0.0f;c_host[i] = 1.1751f;
    }
    ...
    Q.submit([&](handler &h){
        ...
        accessor aB{bufferB,h,read_write};
        accessor aC{bufferC,h,read_write};
        ...
        h.parallel_for<class mm>(1,[=](id<1> i) {
            ...
            for (int j = 0;j < N;++j){
                ...
                __mlvm_stream_sub_f32(cPtr,bPtr,cPtr,N);
    ...
    std::cout << "sinh(1) Result: " << result[0] << " .. " << result[N - 1] << std::endl;
    // for(int i = 0; i<N; i++){

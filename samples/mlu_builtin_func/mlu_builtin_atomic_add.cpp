...
int main(){
    ...
    for(int i = 0;i < N;i++){
        a[i] = 1.0f;b[i] = 2.0f;c[i] = 0.0f;c_host[i] = 3.0f;
    }
    ...
    Q.submit([&](handler &h){
        ...
        accessor aB{bufferB,h,read_only};
        ...
        h.parallel_for<class mm>(1,[=](id<1> i) {
            ...
            for (int j = 0;j < N;++j){
                ...
                __mlvm_stream_atom_add_f32(cPtr,aPtr,bPtr,N);
            #endif
            }
    ...
    std::cout << "atomic add Result: " << result[0] << " .. " << result[N - 1] << std::endl;
    // for(int i = 0; i<N; i++){

...
int main(){
    ...
    for(int i = 0;i < N;i++){
        a[i] = 2.0f;b[i] = 0.0f;c_host[i] = 7.387f;
    }
    ...
    host_accessor result{bufferB,read_only};

    std::cout << "active exp Result: " << result[0] << " .. " << result[N - 1] << std::endl;
    // for(int i = 0; i<N; i++){
    ...
    std::cout << "active exp Time : " << endTime - startTime <<" us "<< std::endl;

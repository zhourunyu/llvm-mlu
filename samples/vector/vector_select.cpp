...
void SelectOp(sycl::queue &q, std::array<T, n> &a, const std::array<T, n> &b, const std::array<T, n> &c, std::array<T, n> &d) {
    ...
    q.parallel_for<class mm>(sycl::range<3>{1, 1, N_CORES}, [=](sycl::item<3> item) {
        ...
        sycl::ext::mlu::vector_select(nram_buffer_b, nram_buffer_a, nram_buffer_b, nram_buffer_c, nram_length);
...
int main() {
    ...
    std::array<float, N> a, b, c, d, d_host;
    initArray(a);
    initArray(b);
    initArray(c);
    for (auto &i : a)
